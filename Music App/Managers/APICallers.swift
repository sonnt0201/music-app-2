...
final class APICallers {
    ...
    // MARK: - CREAT REQUEST FUNC USED FOR EVERY API FETCHING
...
//                    print(result)
    ...
    public func getFeaturedPlaylists (completion: @escaping (Result<FeaturedPlaylistsResponse, Error>) -> Void){
        createRequest(with: URL(string: Constants.baseAPIURL + "/browse/featured-playlists?limit=21"),
                      type: .GET) { request in
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                guard let data = data, error == nil else {
                    print(completion(.failure(APIError.faileedToGetData)))
                    return
                }
                do {
                    let result = try JSONDecoder().decode(FeaturedPlaylistsResponse.self, from: data)
//                    JSONSerialization.jsonObject(with: data, options: .fragmentsAllowed)
//                    print("featured playlist reponse \(result)")
                    completion(.success(result))
                } catch {
                    print(error)
                    completion(.failure(error))
...
//     MARK: - GET RECOMMENDATION API
    public func getRecommendations (genres: Set<String> ,completion: @escaping (Result<RecommendationResponse, Error>) -> Void) {
        let seeds = genres.joined(separator: ",")
        createRequest(with: URL(string: Constants.baseAPIURL + "/recommendations?seed_genres=\(seeds)&limit=40"),
                      type: .GET) { request in
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                guard let data = data, error == nil else {
                    print(completion(.failure(APIError.faileedToGetData)))
...
//                    print(result)
    ...
    // MARK: - GET RECOMMENDED GENRE BEFORE GET RECOMMEDATION
...
//                    print(result)
    ...
    // MARK: - PLAYLISTS
    public func getPlaylistDetails (for playlist: Playlist, completion: @escaping (Result<PlaylistsDetailResponse, Error>) -> Void) {
        createRequest(with: URL(string: Constants.baseAPIURL + "/playlists/" + playlist.id),
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    let result = try JSONDecoder().decode( PlaylistsDetailResponse.self, from: data)
//                    JSONSerialization.jsonObject(with: data, options: .allowFragments)
//                    print(result)
                    completion(.success(result))
                } catch {
...
// TODO: - ALL CATAGORIES RESPONSE
//    public func getCatagory(completion: @escaping (Result<String, Error>) -> Void){
//        createRequest(with: URL(string: Constants.baseAPIURL + "/brower/catagories?limit=2"),
//                      type: .GET) { request in
//            let task = URLSession.shared.dataTask(with: request) { data, _, error in
//                guard let data = data, error == nil else {
//                    completion(.failure(APIError.faileedToGetData))
